...
function totalProduct() {
  ...
  $('.things__single span').each(function(index, item) {
    ...
    let singleNew = $(item).data('new') * Number(input.val());
    let singleOld = $(item).data('old') * Number(input.val());
  ...
  $('#product__current').text(sumNew.toLocaleString('ru'));
}


$(document).on('click', '.things .basket__del', function(e) {
...
$(document).on('click', '.things .things__elem--yes', function(e) {
  let item = $(e.target).parents('.things__col');
  item.remove();
  totalProduct();
})

$(document).on('click', '.things .things__elem--no', function(e) {
  let item = $(e.target).parents('.things__col');
...
function totalProcat() {
  let elems = $('.checkout__count');
  let sum = 0;
  $(elems).each(function(index, item) {
    sum += $(item).data('single');
  })
  $('#procat__sum').text(sum.toLocaleString('ru'));
}

$(document).on('click', '.basket .basket__del', function(e) {
...
$(document).on('click', '.basket .things__elem--yes', function(e) {
  let item = $(e.target).parents('.checkout__item');
  item.remove();
  totalProcat();
})

$(document).on('click', '.basket .things__elem--no', function(e) {
  let item = $(e.target).parents('.checkout__item');
  $(item).removeClass('active');
  let popup = item.find('.things__popup');
  popup.removeClass('things__popup--active');
...
$(document).mouseup(function(e) {
  ...
  if (!div.is(e.target) && div.has(e.target).length === 0) {
   ...
   $('.things__col').removeClass('active');
  }
});
